...
const SES_COLS = '52px 1.3fr 0.7fr 1.2fr 0.7fr 1.6fr 1fr 40px'
const PAGE = 20
...
export default function EnrollSessions() {
  ...
  }, [filterDir, filterRooms, filterName])

  useEffect(() => { load(0) }, [filterDir, filterRooms])

  function closeDrawer(refresh) {
    setDrawer(null)
    if (refresh) { load(offset); reloadHeader?.() }
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', background: 'var(--bg1)', border: '1px solid var(--ln)', borderRadius: 11, padding: '12px 14px', marginBottom: 14 }}>
        ...
        />
        <button onClick={() => load(0)} style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: 7, background: 'var(--in)', color: 'oklch(0.16 0.03 152)', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 12.5, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer' }}>
          <Icon name="search" size={13} />Lọc
        </button>
        <span style={{ fontSize: 11, color: 'var(--txl)' }}>Khớp 1-1 với Gate Log (cùng bộ lọc)</span>
      </div>
      ...
      {loading && !items.length ? (
        ...
        <Card>
          <div style={{ display: 'grid', gridTemplateColumns: SES_COLS, gap: 10, padding: '11px 14px', fontFamily: 'var(--mono)', fontSize: 9.5, letterSpacing: '0.8px', color: 'var(--txl)', textTransform: 'uppercase', borderBottom: '1px solid var(--bg2)' }}>
            <div>Snap</div><div>Thời gian</div><div>Phòng</div><div>Trạng thái</div><div>Chiều</div><div>Người / Nhận diện</div><div>Quality</div><div />
          </div>
          {items.map(r => {
            ...
            return (
              <div key={r.door_id} onClick={() => setDrawer(r.door_id)} style={{ display: 'grid', gridTemplateColumns: SES_COLS, gap: 10, alignItems: 'center', padding: '10px 14px', borderBottom: '1px solid var(--bg1)', cursor: 'pointer' }}>
                <Avatar gender={r.recognized_gender} size={34} src={snapUrl(r.snap_event_id)} />
